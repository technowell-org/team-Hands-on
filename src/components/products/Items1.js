...
export const Items1 = ({ cartItem, setCartItem }) => {
  ...
  const listItem = Items.map((item) => {
    ...
    if (id === item.id) {
      return (
        <div className="renderItems" key={item.id}>
          {/* <Heading current="Products" titles={titles}></Heading> */}
          <div className="image">
            <div className="offer">{item.offer}</div>
            <img className="img" src={item.pic} alt="products"></img>
          </div>
          <div className="content">
            ...
            <hr></hr>
            <button type="button" className="btn"   onClick={onAddCartClick}>
